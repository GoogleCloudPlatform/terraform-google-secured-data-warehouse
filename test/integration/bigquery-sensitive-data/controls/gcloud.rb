...
control 'gcloud' do
  ...
  describe command("gcloud data-catalog taxonomies list --location='us-east4' --project=#{taxonomy_project_id} --format=json") do
    its(:exit_status) { should eq 0 }
  end
  ...
  #
  #   describe command("bq show --schema  --headless --location='us-east4' --project_id=#{taxonomy_project_id} dtwh_dataset.sample_data") do
  #     its(:exit_status) { should eq 0 }
  #     its(:stderr) { should eq '' }
  #     let(:data) do
  #       if subject.exit_status.zero?
