...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Encrypt plaintext using a symmetric key.')
    parser.add_argument('--project_id', dest='project_id',
                        help='project_id (string): Google Cloud project ID (e.g. "my-project").')
    parser.add_argument('--location_id', dest='location_id',
                        help='location_id (string): Cloud KMS location (e.g. "us-east1").')
    parser.add_argument('--key_ring_id', dest='key_ring_id',
                        help='key_ring_id (string): ID of the Cloud KMS key ring (e.g. "my-key-ring").')
    parser.add_argument('--key_id', dest='key_id',
                        help='key_id (string): ID of the key to use (e.g. "my-key").')
    parser.add_argument('--plaintext', dest='plaintext',
                        help='plaintext (string): message to encrypt')

    args = parser.parse_args()
    sys.exit(encrypt_symmetric(args.project_id, args.location_id, args.key_ring_id, args.key_id, args.plaintext))
